...
# Fingertips-y stuff
inds <- indicators()
inds_obesity <- inds[grep('abortion',inds$IndicatorName),]
...
df_to_plot_upper <- fingertips_data(IndicatorID = cts_indicator, AreaTypeID = 102) %>% 
  filter(AreaName %in% areas_of_interest) %>% 
  mutate(Geography = 'Upper')

df_to_plot_lower <- fingertips_data(IndicatorID = cts_indicator, AreaTypeID = 101) %>% 
