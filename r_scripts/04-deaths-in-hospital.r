...
# Fingertips-y stuff
inds <- indicators()
inds_deaths <- inds[grep('death',inds$IndicatorName),]

deaths_indicator <- 93474
...
df_to_plot_upper <- fingertips_data(IndicatorID = deaths_indicator, AreaTypeID = 402) %>% 
  filter(AreaName %in% areas_of_interest) %>% 
  mutate(Geography = 'Upper')

df_to_plot_lower <- fingertips_data(IndicatorID = deaths_indicator, AreaTypeID = 401) %>% 
...
df_plot_all <-rbind(df_to_plot_lower, df_to_plot_upper) %>% 
  ...
  mutate(AreaName = forcats::fct_rev(forcats::fct_inorder(AreaName)),
         ...
         AreaType = forcats::fct_relevel(AreaType, c('England','Region','Counties & UAs (from Apr 2021)','Districts & UAs (from Apr 2021)')))
...
indicator_name <- paste0(indicator_name, " (", df_plot_all$Timeperiod %>% unique(),", ",df_plot_all$Age %>% unique(),")")
...
ggsave(deaths_rag, filename = 'deaths_hospital_rag_bar_ALPHA.png', path = save_place, bg = 'transparent', units = 'mm', width = 180, height = 180)
