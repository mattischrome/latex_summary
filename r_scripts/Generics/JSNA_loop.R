...
library(readxl)
JSNA_indicators <- read_excel("Documents/jsna/JSNA indicators.xlsx") %>% 
  filter(!is.na(XXXX) & `Chart Type` == "Line")


# Fingertips-y stuff
inds <- indicators() 

for (i in 1:nrow(JSNA_indicators)) {

### MY VARIABLES
XXXX <- JSNA_indicators$XXXX[i]
YYYY <- JSNA_indicators$YYYY[i]
ZZZZ <- JSNA_indicators$ZZZ[i]
