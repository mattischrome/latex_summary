...
### MY VARIABLES
XXXX <- 40501
YYYY <- "Under 75 mortality rate<br>from cancer"
ZZZZ <- "u75_cancer_line_3y"
...
caption <- paste0('**',str_replace_all(caption_data[2,3],' ',''),' ',YYYY,'**<br>',caption_data[2,1],' = ',caption_data[2,2],' per 100,000<br>',caption_data[1,1],' = ',caption_data[1,2],' per 100,000')
...
plot <- ggplot(
  ...
  geom_richtext(x='2009 - 11', y=75, label=caption, stat = 'unique', color = 'black', hjust=0, fill = NA, label.color = NA, size = rel(4)) +
...
ggsave(plot, filename = paste0(ZZZZ,".png"),path = save_place, bg = 'transparent', units = 'mm', width = 180, height = 100)
